...
router.post( '/GetTokenCandidate', ( req, res ) => {
    ...
    let q = "SELECT candidate_info.candidate_name, \
    candidate_info.candidate_id, \
    candidate_tokens.token_no \
    FROM candidate_info  \
    INNER JOIN candidate_tokens ON candidate_info.candidate_id = candidate_tokens.candidate_id  \
    WHERE candidate_tokens.token_no = '" + token + "' AND insert_date = '" + date + "'"
