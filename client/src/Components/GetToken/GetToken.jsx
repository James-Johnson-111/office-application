...
class GetToken extends Component {
    ...
    componentDidMount()
    {
        ...
        const tok = () => {
            ...
            axios.get('/gettokens').then( response => {
                ...
                let token = parseInt( response.data[0] ? response.data[0].token : 0 );
    ...
    getToken = ( event ) => {
        ...
        let Url = 'https://192.168.10.116:3000/#/welcomecandidate/' + hashedTokenNo;
        console.log(Url);
        QRcode.toDataURL(Url).then( response => {
            ...
            axios.post( '/storetokens', formsData ).then( () => {
                this.setState(
                    {
                        getTokenContent: 
                        <> 
                            <div style={ { width: "100%", padding: 0 } } id="tokenContent"> 
                                <div style={ { display: "flex", alignItems: "center", justifyContent: "center" } }> 
                                    ...
                                    <div style={ { width: "50%", textAlign: "center", textTransform: "uppercase", fontWeight: "bold" } }> 
                                        <div> 
                                            <span style={ { marginBottom: 0, display: "block" } }>Labofficial</span> 
                                            <span style={{ fontSize: '40px', display: "block", marginBottom: 0 }}> { tokenTXT } </span> 
                                            <span style={{ marginBottom: 0, display: "block" }}> { fullTime } </span> 
                                        </div> 
                                    </div> 
                                </div> 
                            </div> 
